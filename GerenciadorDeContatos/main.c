...
void backupContacts(Contact* head, const char* backupFilename) {
...
Contact* addContact(Contact** head, const char* name, const char* phone) {
    ...
    backupContacts(*head, "backup_contacts.txt");
    return newContact;
}
...
void removeContact(Contact** head, const char* name) {
    ...
    if (strcmp(temp->name, name) == 0) {
        *head = temp->next;
        free(temp);
        saveContacts(*head, FILENAME);
        backupContacts(*head, "backup_contacts.txt"); // Backup após salvar
        return;
    }
...
void restoreContacts(Contact** head, const char* backupFilename) {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 5:
                ...
                break;
            case 6:
                backupContacts(head, backupFilename);
                break;
            case 7:
                restoreContacts(&head, backupFilename);
                break;
            case 8:
                printf("Saindo...\n");
                freeContacts(head);
